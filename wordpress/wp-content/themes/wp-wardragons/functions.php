...
function easy_breadcrumbs() {
  ...
  if ( !is_front_page() ) {
    ...
    echo '<div class="c-breadcrumbs">';
    echo '<div class="c-breadcrumbs__home">';
    echo '<a class="c-breadcrumbs__icon" href="' . get_home_url() . '">';
    echo '<i class="c-icon c-icon--home"></i><span class="u-sr-only">' . $home_title . '</span>';
    echo '</a>';
    echo '</div>';
    echo '<ul id="' . $breadcrums_id . '" class="o-list-inline c-breadcrumbs__list ' . $breadcrums_class . '">';


        if ( is_archive() && !is_tax() && !is_category() && !is_tag() ) {

            echo '<li class="c-breadcrumbs__item item-current item-archive">' . post_type_archive_title($prefix, false) . '</li>';
            ...
            if($post_type != 'post') {
                ...
                echo '<li class="c-breadcrumbs__item item-cat item-custom-post-type-' . $post_type . '"><a class="c-breadcrumbs__link  bread-cat bread-custom-post-type-' . $post_type . '" href="' . $post_type_archive . '" title="' . $post_type_object->labels->name . '">' . $post_type_object->labels->name . '</a></li>';

            }

            $custom_tax_name = get_queried_object()->name;
            echo '<li class="c-breadcrumbs__item item-current item-archive">' . $custom_tax_name . '</li>';
            ...
            if($post_type != 'post') {
                ...
                echo '<li class="c-breadcrumbs__item item-cat item-custom-post-type-' . $post_type . '"><a class="c-breadcrumbs__link  c-breadcrumbs__link bread-cat bread-custom-post-type-' . $post_type . '" href="' . $post_type_archive . '" title="' . $post_type_object->labels->name . '">' . $post_type_object->labels->name . '</a></li>';
            ...
            if(!empty($category)) {
                ...
                foreach($cat_parents as $parents) {
                    $cat_display .= '<li class="c-breadcrumbs__item item-cat">'.$parents.'</li>';
                }
            ...
            if(!empty($last_category)) {
                echo $cat_display;
                echo '<li class="c-breadcrumbs__item item-current item-' . $post->ID . '">' . get_the_title() . '</li>';

            // Else if post is in a custom taxonomy
            } else if(!empty($cat_id)) {

                echo '<li class="c-breadcrumbs__item item-cat item-cat-' . $cat_id . ' item-cat-' . $cat_nicename . '"><a class="c-breadcrumbs__link  bread-cat bread-cat-' . $cat_id . ' bread-cat-' . $cat_nicename . '" href="' . $cat_link . '" title="' . $cat_name . '">' . $cat_name . '</a></li>';
                echo '<li class="c-breadcrumbs__item item-current item-' . $post->ID . '">' . get_the_title() . '</li>';
            ...
            // Category page
            echo '<li class="c-breadcrumbs__item item-current item-cat">' . single_cat_title('', false) . '</li>';
            ...
            if( $post->post_parent ){
                ...
                foreach ( $anc as $ancestor ) {
                    $parents .= '<li class="c-breadcrumbs__item item-parent item-parent-' . $ancestor . '"><a class="c-breadcrumbs__link  bread-parent bread-parent-' . $ancestor . '" href="' . get_permalink($ancestor) . '" title="' . get_the_title($ancestor) . '">' . get_the_title($ancestor) . '</a></li>';
                }
            ...
            echo '<li class="c-breadcrumbs__item item-current item-tag-' . $get_term_id . ' item-tag-' . $get_term_slug . '">' . $get_term_name . '</li>';
            ...
            // Year link
            echo '<li class="c-breadcrumbs__item item-year item-year-' . get_the_time('Y') . '"><a class="c-breadcrumbs__link  bread-year bread-year-' . get_the_time('Y') . '" href="' . get_year_link( get_the_time('Y') ) . '" title="' . get_the_time('Y') . '">' . get_the_time('Y') . ' Archives</a></li>';

            // Month link
            echo '<li class="c-breadcrumbs__item item-month item-month-' . get_the_time('m') . '"><a class="c-breadcrumbs__link  bread-month bread-month-' . get_the_time('m') . '" href="' . get_month_link( get_the_time('Y'), get_the_time('m') ) . '" title="' . get_the_time('M') . '">' . get_the_time('M') . ' Archives</a></li>';

            // Day display
            echo '<li class="c-breadcrumbs__item item-current item-' . get_the_time('j') . '"> ' . get_the_time('jS') . ' ' . get_the_time('M') . ' Archives</li>';
            ...
            // Month display
            echo '<li class="c-breadcrumbs__item item-month item-month-' . get_the_time('m') . '">' . get_the_time('M') . ' Archives</li>';

        } else if ( is_year() ) {

            // Display year archive
            echo '<li class="c-breadcrumbs__item item-current item-current-' . get_the_time('Y') . '">' . get_the_time('Y') . ' Archives</li>';
            ...
            // Display author name
            echo '<li class="c-breadcrumbs__item item-current item-current-' . $userdata->user_nicename . '">' . 'Author: ' . $userdata->display_name . '</li>';

        } else if ( get_query_var('paged') ) {

            // Paginated archives
            echo '<li class="c-breadcrumbs__item item-current item-current-' . get_query_var('paged') . '">'.__('Page') . ' ' . get_query_var('paged') . '</li>';

        } else if ( is_search() ) {

            // Search results page
            echo '<li class="c-breadcrumbs__item item-current item-current-' . get_search_query() . '">Search results for: ' . get_search_query() . '</li>';

        } elseif ( is_404() ) {

            // 404 page
            echo '<li class="c-breadcrumbs__item">' . 'Error 404' . '</li>';
        }

        echo '</ul>';
        echo '</div><!-- /.c-breadcrumbs -->';
...
// Add Dragon Post Type
add_action( 'init', 'post_type_dragon' );
function post_type_dragon() {

  $labels = array(
    'name' => 'Dragon',
    'singular_name' => 'Dragon',
    'add_new' => 'Add',
    'add_new_item' => 'Add',
    'edit' => 'Edit',
    'edit_item' => 'Edit',
  ...
  $args = array(
    ...
    'has_archive' => true,
    'rewrite' => array( 'slug' => 'gragon' ),
    // https://developer.wordpress.org/resource/dashicons/
    'menu_icon' => 'dashicons-twitter',
    'show_in_rest' => true
  );

  register_post_type( 'gragon' , $args );
}
